...
- **TikTokPostsActor**: accepts links, hashtags, and text queries in `search_params`; syntax inference sends URLs as `postURLs`, values starting with `#` as `hashtags`, and other values as `searchQueries`. `country_id` maps to Apify `proxyCountryCode` (`_484` → `MX`); when `country_id` is omitted, no proxy country is sent, and unknown country IDs raise an error. Direct Apify input overrides are supported through `actor_params` except for `proxyCountryCode` (e.g. `searchSection`, `shouldDownloadVideos`, `shouldDownloadSubtitles`, `videoSearchSorting`). Searches default to `videoSearchSorting: "LATEST"` — TikTok search is relevance-sorted otherwise, which returns mostly old posts that a task's `period` window then pays for and drops (measured: 84% waste on a 7-day window); override via `actor_params` if relevance order is ever wanted. `get_comments=true` runs `clockworks/tiktok-comments-scraper` after filtering with `commentsPerPost=max_comments` and joins comments to posts by `videoWebUrl`; `get_comments_after_likes` only scrapes comments for posts whose `likes` exceed the threshold.
...
**Filter cache** (`cache/filter_cache.json`): Filtering results are cached per document per task (keyed by `filtered:{task_id}:{url}`). On subsequent runs, documents previously filtered out are skipped before entering the pipeline. The same document can have different outcomes for different tasks. Set `override_filters=true` (per task) to ignore the cache and re-run all filters. To dismiss the cached filtered-out documents for **all** actors/tasks in a run, set the `OVERRIDE_FILTERED_CACHE` constant in `src/run_searches.py` to `True` — it forces `override_filters=True` into every task's actor kwargs.

**LLM-filter decision cache** (`cache/llm_core`): The LLM filter caches each document's keep/drop decision per `(url, condition, snippet_max_len)` (cache tag `llm_filter`, field `keep`). On every run, documents with a cached decision are resolved up front and only the undecided ones are sent to the LLM (batched `LLM_FILTER_BATCH_SIZE=20` at a time); each new decision is written back. Because the key includes `url`, a decision is reused regardless of which batch the document later falls into and is shared across tasks using the same condition. Changing the `condition` (or the second pass's larger `snippet_max_len`) yields a different key, so it re-evaluates automatically. URL-less documents are never cached (always re-sent). `override_filters=true` bypasses the cache read and re-evaluates every document (decisions are still written back). The underlying batch call also keeps a content-addressed cache (tag `llm_filter_batch`, keyed on the batch's snippet text + condition) as a fallback, mainly covering URL-less documents.
...
| 4 | tiktok_posts | #lubricantes,totalenergies | _484 | es | | d | 100 | true | true | true | 5 | | elimina publicaciones no relacionadas con lubricantes | | false | {"searchSection":"/video"} |
...
- **`tiktok/posts.py`** — `TikTokPostsActor` (actor `GdWCkxBtKWOsKjdch`). Scrapes TikTok videos from post URLs, hashtags, or search queries; comments are scraped after filtering via `clockworks/tiktok-comments-scraper` and joined to each post by `videoWebUrl`
...
| `test_tiktok_actor.py` | TikTokPostsActor: syntax inference for links/hashtags/queries, mixed input runs, Apify param overrides, dedicated comments actor enrichment, TikTokPost mapping, final schema |
